...
let rec typecheck(m: ctx) (p: process): localType err =
  ...
  | Send(par,e,p1)      -> let te  = typecheckExpr m e in
                           ...
                           (
                             match (te,tp1) with
                             | (Yes t1, Yes t2) -> Yes(LSend(par,t1,t2))
                             | _                -> No "ill-typing in send"
                           )
  | Receive(par,(EVar x),p1) -> let tx = lookupS m x in
                                (
                                  match tx with
                                  | Yes tx' -> 
                                  ...
                                  (
                                    match tp1 with
                                    | Yes tp1' -> Yes (LReceive(par,tx',tp1'))
  ...
  | Selection(par,l,p1) -> let tp1 = typecheck m p1 in
                           (
                            match tp1 with
                            | Yes tp1' -> Yes(LSelection(par,[(l,tp1')]))
...
and typecheckProcList m pl acc =
  ...
  | (l,p)::xs -> let tp = typecheck m p in
                 (
                  match tp with
                  | Yes tp' -> typecheckProcList m xs (acc @ [(l,tp')])
